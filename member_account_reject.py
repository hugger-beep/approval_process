...
def send_sns_notification(token, timestamp):
    try:
        logger.info("Preparing rejection SNS notification")
        config = Config(
            connect_timeout=2,
            read_timeout=2,
            retries={'max_attempts': 2}
        ...
        response = sns.publish(
            TopicArn=os.environ['REJECTION_TOPIC_ARN'],
            Message=message,
            Subject='Patching Request Rejected'
        )
...
def generate_html_response(success=True, error_message=None):
    if success:
        ...
        <html>
            ...
            <body>
                <div class="container">
                    <h1 class="rejection">Rejection Processed</h1>
                    <div class="message">
                        <p>The patching request has been rejected successfully.</p>
                        <p>The security team has been notified.</p>
